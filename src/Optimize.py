...
import numpy as np
import copy
from Utils import getDefField, combineDeformationFields, deformWholeImage, deformImage, saveImg, getReceptiveFieldOffset, sampleImgData, getPaddedData, sampleImg
...
#         self.net.eval()
  ...
  def trainTestNetDownSamplePatch(self, dataloader):
      ...
      for i, data in enumerate(dataloader, 0):
        ...
        for samplingRateIdx, samplingRate in enumerate(samplingRates):
          numberOfiterations = self.userOpts.numberOfEpochs[samplingRateIdx]
...
#             self.net.reset_params()
            ...
            while True:
              ...
              if lossCounter == 9:
                ...
                if len(landmarksBeforeDeformation) > 0 and self.userOpts.logLandmarkDistance:  # @IndentOk
                  ...
                  tmpDefField = currVectorField * upSampleRate
