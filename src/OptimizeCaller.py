...
import numpy as np
import copy
from Utils import getDefField, combineDeformationFields, deformWholeImage, deformImage, getReceptiveFieldOffset, sampleImgData, getPaddedData, sampleImg
...
#delete       
  ...
  def getDownSampleRates(self):
    ...
    return samplingRates[0:self.userOpts.stoptAtSampleStep]
      ...
      ##deprecated
      ## delete same method is also in Optimise.py
...
#         self.net.eval()
  ...
  def trainTestNetDownSamplePatch(self, dataloader):
      ...
      for i, data in enumerate(dataloader, 0):
        ...
        oneShotOptimiser = OneShotOptimise(data, self.net, self.userOpts)
        currVectorField = oneShotOptimiser.run(dataloader.dataset.getSpacingXZFlip(i))
